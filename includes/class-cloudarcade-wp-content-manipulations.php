...
class Cloudarcade_Wp_Content_Manipulations {
	...
	/**
	 * output for gameee page - list of all games used as shortcode
	 */
	function my_custom_content_after_page($content) {
		...
		if ( is_page('gamee') && in_the_loop() && is_main_query() ) {
			...
			add_filter( 'the_content', 'my_custom_content_after_page' );
	...
	function append_single_game_content( $content ) {
		...
		if ( is_single() && in_the_loop() && is_main_query() ) {
			if ( get_post_type() === 'game' ) {
				remove_filter( 'the_content', 'my_custom_content_game' );
				ob_start(); // start output buffering
				include CLOUDARCADE_WP_ROOT. 'templates/content-single-game.php';
				$output = ob_get_clean(); // end output buffering and capture the output
				add_filter( 'the_content', 'my_custom_content_game' );
				$content = $output; // append the output to the existing content
				// Your custom content
